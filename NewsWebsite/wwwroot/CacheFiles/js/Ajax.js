...
$(function () {
    var placeholder1 = $("#modal-placeholder1");
    $(document).on('click', 'button[data-toggle="ajax-modal1"]', function () {
        var url = $(this).data('url');
        $.ajax({
            url: url,
            /*beforeSend: function () { ShowLoading(); },*/
            ...
            placeholder1.find('.modal').modal('show');
        });
    });

    placeholder1.on('click', 'button[data-save="modal"]', function () {
        ShowLoading();
        var form = $(this).parents(".modal").find('form');
        var actionUrl = form.attr('action');
        ...
        $.ajax({
            ...
            var newBody = $(".modal-body", data);
            var newFooter = $(".modal-footer", data);
            placeholder1.find(".modal-body").replaceWith(newBody);
            placeholder1.find(".modal-footer").replaceWith(newFooter);
    ...
    $(document).on('click', 'a[data-toggle="ajax-modal1"]', function () {
        var url = $(this).data('url');
        $.ajax({
            url: url,
            beforeSend: function () { $('#modal-placeholder1').after('<div class="preloader d-flex align-items-center justify-content-center"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div></div>'); },
            complete: function () { $('.preloader').remove(); },
            error: function () {
                ShowSweetErrorAlert();
            }
